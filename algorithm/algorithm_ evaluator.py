...
cv_preds_base = rfc_base.predict(x_cv)
print("cv_preds : ", cv_preds_base)
# pred_probality = rfc.predict_proba(x_test)
# print("Predict probability : ", pred_probality)
baseline_metrices = evaluate_preds(rfc_base, x_cv, y_cv, cv_preds_base)
# ##############################################
rfc_new = RandomForestClassifier(n_estimators=100)
rfc_new.fit(x_train, y_train)
cv_preds = rfc_new.predict(x_cv)
print("cv_preds : ", cv_preds)
# pred_probality = rfc.predict_proba(x_test)
# print("Predict probability : ", pred_probality)
rfc_new_matrices = evaluate_preds(rfc_new, x_cv, y_cv, cv_preds)
print("Model parameters", rfc_new.get_params())
...
gs_rfc_matrices = evaluate_preds(gs_rfc, x_cv, y_cv, cv_preds)
"""
